...
void processExprNode(AST_NODE* exprNode)
{
    ...
    else
    {
        ...
        if (exprNode->dataType != ERROR_TYPE && 
            exprNode->semantic_value.exprSemanticValue.op.unaryOp == UNARY_OP_LOGICAL_NEGATION) {
            exprNode->dataType = INT_TYPE;
        }
...
void declareFunction(AST_NODE* declarationNode)
{
    ...
    if(errorOccur && enterFunctionNameToSymbolTable)
    {
        ...
        {
            functionNameID->semantic_value.identifierSemanticValue.symbolTableEntry = NULL;
